...
nav_choice = st.sidebar.radio(
    'Navigation', ('Home', 'Sleep Detection', 'Help Us Improve'), index=0)
# Home page
if nav_choice == 'Home':
    st.header(
    ...
    st.markdown('<h1>How to use?<br></h1>'
                ...
                'target="_blank">here</a></font></b>', unsafe_allow_html=True)
...
# Sleep Detection page
    ...
    cap = 0
    st.success(
        'Please look at your web-cam, while following all the instructions given on the Home page.')
    st.warning(
    ...
    if start == 'Start':
        ...
        for _ in range(3):
            ...
            try:
                ...
                cv2.rectangle(frame1, (x + centx - 60, y + centy - 40),
                              (x + centx - 10, y + centy), (0, 255, 0), 5)
                cv2.rectangle(frame1, (x + centx + 10, y + centy - 40),
                              (x + centx + 60, y + centy), (0, 255, 0), 5)
                ...
                img_container[2].image(cv2.cvtColor(
                    eye_1, cv2.COLOR_BGR2RGB), width=150)
                img_container[3].image(cv2.cvtColor(
                    eye_2, cv2.COLOR_BGR2RGB), width=150)
                print(e1, e2)
...
# Help Us Improve page
    ...
    st.markdown(
        ...
        '<font face="Comic sans MS">better, and to use it in real-time situations, we require as much data as we can gather.</font> ', unsafe_allow_html=True)
    st.warning(
        'NOTE: Your identity will be kept anonymous, and only your eye-patch will be extracted!!!')
    # Image upload
    ...
    if img_upload is not None:
        ...
        to_add.to_csv('Data_from_users.csv', mode='a',
